import 'badge_item.dart';
import 'shields_badge.dart';

Badge mediumBadge(String link, BadgeStyle style) {
  return Badge(
    altText: "Medium",
    link: link,
    badgeContent: SimpleShieldsBadge(
      style: style,
      logo: const ShieldsBadgeImageLogo("Medium"),
      color: "blue",
      leftText: "Medium",
      rightText: "Read",
    ),
  );
}

Badge travisCIBadge(String user, String repo, BadgeStyle style,
    {String branch = "master"}) {
  return Badge(
    altText: "Travis CI",
    link: "https://travis-ci.com/$user/$repo",
    badgeContent: SmartShieldsBadge(
      link: "https://img.shields.io/travis/com/$user/$repo/$branch",
      style: style,
      logo: const ShieldsBadgeBase64Logo(
          "data:image/png;base64,"),
    ),
  );
}

Badge codecovBadge(String user, String repo, BadgeStyle style,
    {String branch = "master", String vcsName = "github"}) {
  return Badge(
    altText: "Codecov",
    link: "https://codecov.io/gh/$user/$repo",
    badgeContent: SmartShieldsBadge(
      link: "https://img.shields.io/codecov/c/$vcsName/$user/$repo/$branch",
      style: style,
      logo: const ShieldsBadgeBase64Logo(
          "data:image/png;base64,"),
    ),
  );
}

Badge githubLicenseBadge(String user, String repo, BadgeStyle style) {
  return Badge(
    altText: "License",
    link: "https://github.com/$user/$repo/blob/master/LICENSE",
    badgeContent: SmartShieldsBadge(
      link: "https://img.shields.io/github/license/$user/$repo",
      style: style,
      logo: const ShieldsBadgeImageLogo("github"),
    ),
  );
}

Badge pubDevBadge(String package, BadgeStyle style) {
  return Badge(
    altText: "License",
    link: "https://pub.dartlang.org/packages/$package",
    badgeContent: SmartShieldsBadge(
      link: "https://img.shields.io/pub/v/$package.svg",
      style: style,
      logo: const ShieldsBadgeBase64Logo(
          "data:image/png;base64,"),
    ),
  );
}

Badge githubStarsBadge(String user, String repo, BadgeStyle style) {
  return Badge(
    altText: "Github Stars",
    link: "https://github.com/$user/$repo",
    badgeContent: SmartShieldsBadge(
      link: "https://img.shields.io/github/stars/$user/$repo",
      style: style,
      logo: const ShieldsBadgeImageLogo("github"),
    ),
  );
}

Badge twitterBadge(String user, BadgeStyle style) {
  return Badge(
    altText: "Twitter Follow",
    badgeContent: SmartShieldsBadge(
      link: "https://img.shields.io/twitter/follow/$user",
      style: style,
      logo: const ShieldsBadgeImageLogo("twitter"),
    ),
  );
}

Badge githubFollowersBadge(String user, BadgeStyle style) {
  return Badge(
    altText: "Twitter Follow",
    badgeContent: SmartShieldsBadge(
      link: "https://img.shields.io/github/followers/$user",
      style: style,
      logo: const ShieldsBadgeImageLogo("github"),
    ),
  );
}
